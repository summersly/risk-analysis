// components/rvd-cards/framingham-card/framingham-card.js
import framinghamModel from 'framinghamModel'
Component({
  /**
   * 组件的属性列表
   */
  ...
  methods: {
    ...
    calcResult: function () {
      ...
      if (toastMessage) {
        wx.showToast({
          title: toastMessage.slice(0, -1) + ' 尚未输入',
          icon: 'none',
          duration: 1500
        })
        return false
      }
      if (this.data.ageModel.isWarning) {
        wx.showToast({
          title: '年龄不在评估范围内',
          icon: 'none',
          duration: 1500
      ...
      console.log(lable)
      //以mg/dl为计算单位
      tcValue = (tcunitIndex === 0) ? (tcValue * 38.61) : tcValue
      hdlValue = (hdlunitIndex === 0) ? (hdlValue * 38.61) : hdlValue
      let riskResult = framinghamModel.calculate(ageValue, tcValue, hdlValue, sbpValue, dbpValue, sex, smoker, diabetes,lable)
      this.setData({
